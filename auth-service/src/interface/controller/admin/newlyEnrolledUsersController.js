...
export class NewlyEnrolledUserController {
  constructor(dependencies) {
    this.newlyEnrolledUserUseCase =
      new dependencies.useCase.NewlyEnrolledUserUseCase(dependencies);
  }
  async newlyEnrolledUsers(req, res, next) {
    try {
      ...
      const getNewUsersReport = await this.newlyEnrolledUserUseCase.execute(
        filter
      );
